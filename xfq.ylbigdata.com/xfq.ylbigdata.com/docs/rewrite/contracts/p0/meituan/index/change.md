# [meituan] ANY /meituan/index/change

## 1. 基本信息

- 路径（兼容要求）：`/meituan/index/change`（建议同时兼容：`/meituan/index/change.html`）
- 源码定位：`app/meituan/controller/Index.php:52`
- 控制器/方法：`meituan/Index.change()`
...
- 正常用例：携带合法 BA Header，请求返回固定 JSON 字符串（`code=200,describe=成功`）
