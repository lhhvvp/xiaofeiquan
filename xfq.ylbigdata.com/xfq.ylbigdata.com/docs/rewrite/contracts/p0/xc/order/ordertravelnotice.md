# [xc] GET /xc/order/OrderTravelNotice

## 1. 基本信息

- 路径（兼容要求）：`/xc/order/OrderTravelNotice`（建议同时兼容：`/xc/order/OrderTravelNotice.html`）
- 源码定位：`app/xc/controller/Order.php:840`
- 控制器/方法：`xc/Order.OrderTravelNotice()`
- 描述：出行通知（测试接口，触发向携程发起出行通知；直接 `var_dump` 输出）
