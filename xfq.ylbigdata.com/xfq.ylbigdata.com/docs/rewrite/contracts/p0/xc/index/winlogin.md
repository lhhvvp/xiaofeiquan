# [xc] POST /xc/index/winlogin

## 1. 基本信息

- 路径（兼容要求）：`/xc/index/winlogin`（建议同时兼容：`/xc/index/winlogin.html`）
- 源码定位：`app/xc/controller/Index.php:102`
- 控制器/方法：`xc/Index.winlogin()`
...
- 正常用例：参数齐全且验证码正确，返回 `code=0,msg=登录成功` 且含 `token`
- 异常用例：缺参/验证码错误/密码错误/账号锁定，返回 `code!=0` 且 `msg` 对应
