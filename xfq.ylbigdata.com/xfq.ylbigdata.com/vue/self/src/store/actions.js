...
import {login, getTicketPirce, paySubmit, orderList,orderDetail,refundAll,refundOne,statistics,checkOrderStatus,orderListSearch,orderListIdcard,takeTicket} from "@/api/user"
...
const actions = {
    ...
    checkOrderStatus(context, data) {
        return new Promise((resolve, reject) => {
            checkOrderStatus(data).then(response => {
                resolve(response)
            }).catch(error => {
                reject(error)
            })
