      ...
      <template #default>
        <div style="height: 650px;overflow: auto">
          <div class="info-item">
            ...
            <div class="item-info">
              <div class="item-info-lable">订单状态：</div>
              <div class="item-info-centent">{{ orderDetail.order_status_text }}</div>
            </div>
            <div class="item-info">
              <div class="item-info-lable">退款状态：</div>
              <div class="item-info-centent">{{ orderDetail.refund_status_text }}</div>
            </div>
          </div>
          <div class="info-item" v-for="(item,index) in orderDetail.detail" :key="index">
...
<!--            <div class="item-info">-->
<!--              <div class="item-info-lable">可用门票数量：</div>-->
<!--              <div class="item-info-centent">{{ item.ticket_number }}张</div>-->
<!--            </div>-->
            <div class="item-info">
              <div class="item-info-lable">门票时间：</div>
              <div class="item-info-centent">{{ item.ticket_date }}
                <el-text class="mx-1" type="primary" v-if="item.enter_time > 0">(已入园)</el-text>
              </div>
            </div>
            <div class="item-info">
...
<!--            </div>-->
            ...
            <div class="item-info" v-if="item.refund_time">
              ...
              <div class="item-info-centent">{{ item.refund_time_text }}</div>
            </div>
            <div class="item-info-blcok" v-if="item.refund_status !== 'fully_refunded' && item.enter_time === 0">
      ...
      <template #footer>
        <div class="dialog-footer">
          ...
          <el-button type="danger" @click="refund()" v-if="orderDetail.refund_status === 'not_refunded' && orderDetail.order_status ==='paid'">
            全部退款
          </el-button>
        </div>
      </template>
...
const refundSubmit = () => {
  ...
  if (refundVal.out_trade_no == null) {
    //全部
    let option = {
      out_trade_no: orderDetail.value.out_trade_no, //订单号
      refund_desc: refundVal.refund_desc, //备注
      uuid: userinfo.value.uuid,  //uuid
    };
    ...
    //单条
    let option = {
      out_trade_no: refundVal.out_trade_no, //订单号
      ticket_number: refundVal.number,  //数量
      refund_desc: refundVal.refund_desc,//备注
      uuid: userinfo.value.uuid,//uuid
    };
    // if (!refundVal.number) {
