...
class Data extends Base
{
    ...
    public function accounting_form(){
        ...
        if (Request::isPost()) {
            ...
            if (true !== $result) {
                ...
                // 根据团ID查询信息
                $tourInfo = \app\common\model\Tour::where('id','in',explode(',',$data['write_off_ids']))->with(['tourist'])->select();
                // 统计游客数量
                $tourist_total  = 0;
                ...
                foreach ($listData as $key => $value) {
                    ...
                    <table width="800" border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#999999" class="xxxx">
                      <tr>
                        ...
                        <td width="120" align="center">手机号</td>
                        <td width="120" align="center">领取时间</td>
                        <td width="70" align="center">消费券面额</td>
                        <td width="70" align="center">合同</td>
                        <td width="70" align="center">保单</td>
                        <td width="70" align="center">打卡次数</td>
                        <td width="120" align="center">核销时间</td>
                      </tr>';
                ...
                if (!$accId) {
                    ...
                    // 触发事件
                    \app\common\model\Tour::where('id','in',explode(',',$data['write_off_ids']))->update(['tour_accounting_id'=>$accId,'accounting_time'=>time()]);
                    \app\common\model\TourWriteOff::where('tid','in',explode(',',$data['write_off_ids']))->where('type',1)->update(['accounting_id'=>$accId,'update_time'=>time()]);
    ...
    {   
        $id = Request::param('id');
        $accInfo = \app\common\model\TourAccounting::where('id',$id)->find();
