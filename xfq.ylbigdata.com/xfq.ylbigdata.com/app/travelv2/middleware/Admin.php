 ...
 */
namespace app\travel\middleware;
...
class Admin
{
    ...
    {   
        $seller_id = Session::get('travel.id');
        $class_id  = Session::get('travel.class_id');
        if(!$seller_id || !$class_id){
            Session::delete('travel');
            return redirect((string)url('login/index'));
        }
        // 获取当前用户
        if(isset($seller_id))
            $seller = \app\common\model\Seller::where('id',$seller_id)->find();
        else
            $seller = [];
        ...
        // 已登录自动跳转并且验证完成才能登录
        if ($seller['email_validated'] !=1 || $seller['status'] !=1){
            return redirect((string)url('/seller/register/index'));
        }
        // 定义方法白名单
        $allow = [
            'Index/index',      // 首页
        ...
        if (Request::get("ref") == 'tab') {
            ...
            return redirect('travel/Index/index')->with('referer', $url);
        }
