...
/**
 * 商户管理模型
 * @author slomoo <slomoo@aliyun.com> 2022-07-22
 */
...
class Seller extends Base
{
    ...
    public static function checkLogin()
    {
        ...
        if ($result['class_id']==3) {
            $data = ['error' => 3, 'msg' => '检测到您是旅行社商户，请移步旅行社登录窗口进行操作', 'href' => url('travel/login/index')->__toString()];
            return json($data);
        }

        $check = Request::checkToken('__seller__token__');
        if (false === $check) {
            $data = ['error' => '2', 'href' => url('login/index')->__toString(), 'msg' => '令牌有误，刷新后重试'];
            return json($data);
        }
        ...
        if ($result['status'] == 1) {
            ...
            Session::set('seller.group_id', $rules['group_id']);
            Session::set('seller.rules', explode(',', $rules['rules']));
            Session::set('seller.title', $rules['title']);
            ...
            $data = ['error' => 2, 'href' => url('Register/index')->__toString(), 'msg' => '审核中'];
            return json($data);
        } else {
