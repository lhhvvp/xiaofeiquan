...
class MakeBuilder
{
    ...
    {   
        // 2023-09-13 适配达梦数据库
        $databaseType = env('database.type');
        $length = ($databaseType === 'dm') ? 4 : 3;
        
        $result = [];
        // 当元素个数为3时执行tree操作
        if ($array && count(($array[0])) == $length) {
    ...
    private function makeReplace(string $content, string $tableName)
    {
        ...
        $content = str_replace('{$author}', 'slomoo', $content);
        $content = str_replace('{$email}', '1103398780@qq.com', $content);
