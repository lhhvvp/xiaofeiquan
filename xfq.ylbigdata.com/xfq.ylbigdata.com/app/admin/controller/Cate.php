...
class Cate extends Base
{
     ...
     * @return bool
     */
    private function cateRuleInit(array $data = [])
    {
        // 根据moudle_id查询对应的栏目所有操作【该操作在生成模块时候自动选择】，并格式化为权限ID
        $module = \app\common\model\Module::where('id', $data['module_id'])->find()->toArray();
        ...
        if ($module) {
            ...
            $cateRule = \app\common\model\CateRule::create($ruleData);
            
            // 集合所有操作权限
            $top_button   = explode(',',$module['top_button']);
            $right_button = explode(',',$module['right_button']);
            $arr = array_keys(array_flip($top_button)+array_flip($right_button));
            foreach ($arr as $key => $value) {
                switch ($value) {
                    case 'add':
                        array_push($arr,'addPost');
                        break;
                    case 'edit':
                        array_push($arr,'editPost');
                        break;
                    // 剔除废弃权限
                    case 'delete':
                        unset($arr[$key]);
            ...
            $param  = [];
            for ($i=0; $i < $len; $i++) {
                $param[$i]['module_id'] = $data['module_id'];
                $param[$i]['pid']       = $cateRule->id;
                $param[$i]['name']      = $module['model_name'].'/'.$arr[$i];
                $param[$i]['title']     = $ruleButton[$arr[$i]];
                $param[$i]['type']      = 1;
                $param[$i]['status']    = 0;
                $param[$i]['auth_open'] = 1;
            ...
            // 返回受影响行数
            $a = Db::name('CateRule')->insertAll($param);
            return true;
        }
        return false;
    }
