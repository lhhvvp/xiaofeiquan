...
class Upload extends Base
{
    ...
    private function bigUpload()
    {
        ...
        if ($done) {
            ...
            // 上传驱动
            $uploadPath = $this->uploadDriver($uploadPath,$_FILES["file"]);
    ...
    private function uploadDriver(string $fineName = '')
    {
        if ($fineName) {
            ...
            switch ($uploadDriver) {
                ...
                case '2':
                    // 阿里云OSS上传
                    return alioss($fineName,'admins',true,'wlxfq');
                case '3':
                    // 七牛云上传
                ...
                case '4':
                    // 私有oss
                    $newStr = extractString($fineName, '/', '.').'.'.pathinfo($_FILES['file']['name'], PATHINFO_EXTENSION);
                    $file = $_SERVER['DOCUMENT_ROOT'].$fineName;
                    $date = substr($fineName, 9, 8);
                    $category = '/admin/'.$date.'/';
                    $fileurl = slomoooss($newStr, $file, $category);
                    return $fileurl;
            }
        }
    }
