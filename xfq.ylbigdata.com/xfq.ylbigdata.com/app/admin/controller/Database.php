...
class Database extends Base
{
    ...
    {
        // 设置主键
        $pk = $this->dbType=='dm' ? 'table_name' : 'name';
        // 字段信息
        $columns = [
            ['name'  , '数据表'],
            ['engine', '存储引擎'],
        ...
        $total = 0;
        foreach ($list as $k => $v) {
            $length = $this->dbType == 'dm' ? $v['bytes'] : $v['data_length'];
            $total += $length;
            $list[$k]['data_size'] = $length;
            $list[$k]['data_length'] = format_bytes($length);
            $list[$k]['name'] = $v[$pk];
            $list[$k]['rows'] = $this->dbType=='dm' ? $v['num_rows'] : $v['rows'];
        }
        // 提示信息
