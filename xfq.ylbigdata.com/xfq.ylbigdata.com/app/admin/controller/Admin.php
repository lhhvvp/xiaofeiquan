...
class Admin extends Base
{
    ...
    private function getAuthGroupOptions()
    {
        ...
        $authGroup = \app\common\model\AuthGroup::where('status', '=', 1)
            ->where('id','>=',Session::get('admin')['group_id'])
            ->select()
            ->toArray();
        $groupOptions = [];
    ...
    public function editpassPost()
    {
        if (Request::isPost()) {
            ...
            // 校验原始密码是否正确
            $admininfo = \app\common\model\Admin::find($data['id']);
            if($data['password']!=$admininfo['password']){
                $result = ['error' => '1', 'msg' => '原始密码错误'];
                return json($result);
            }
            ...
            if (false === $check) {
                $result = ['error' => '1', 'msg' => '请勿重复操作'];
                return json($result);
            }
            $data['password'] = md5($newpassword);
            \app\common\model\Admin::update($data, $where);
            Session::delete('admin');
            $result = ['error' => '0', 'href' => url('Login/logout')->__toString(), 'msg' => '修改成功,请重新登录'];
            return json($result);
        }
    }
}
