...
class Admin
{
    public function handle($request, \Closure $next)
    {
        // 15分钟 长时间无操作  退出登录
        $nowtime = time(); 
        $exp_access_time = Session::get('exp_access_time');
        if(isset($exp_access_time) && $nowtime - $exp_access_time > 86400){
            Session::delete('exp_access_time');
            Session::delete('admin');
            return redirect('/admin/index/index');
        }else{
            Session::set('exp_access_time',$nowtime);
        }
