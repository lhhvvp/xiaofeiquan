	...
	<view v-show="value" class="calendar-body">
		<view class="calendar">
			<view class="title-name">
				{{title}} <uni-icons class="icon-close-bold" type="closeempty" @click="close()"></uni-icons>
			</view>
			...
			<scroll-view class="day-list" :class="{'day-range': mode == 'range'}" scroll-y="true"
				:scroll-top="scrollTop" @scrolltolower="changeMonthHandler(1)" style="width: 100%;">
				<view class="day-item" v-for="item in listData" :key="item.showTitle">
					<view class="day-title">{{item.showTitle}}</view>
					<view class="week-list">
						<view class="week-day__text" v-for="(itemZh, indexZh) in weekDayZh" :key="indexZh">{{itemZh}}
						</view>
					</view>
					...
					<view class="content">
						...
						<view class="item" v-for="(dayItem, dayIndex) in item.daysArr" :key="dayIndex" :class="{ 
							...
							<view class="dec">{{ viewDayAltStock(item.year, item.month, dayIndex+1) }}</view>
						</view>
					</view>
				</view>
			</scroll-view>
	...
	export default {
		...
		methods: {
			...
			openDisAbled(year, month, day) {
				...
				bool = false;
				this.altPrice.forEach(item => {
					if (item.date == BoolDate) {
						bool = true;
					}
				})
				// 禁止周六周日下单
			...
			// 显示日期下的文字提示
			viewDayAlt(year, month, day, is = true) {
				let txt = ''
				...
				if (item && item.price) {
					if (Number.isFinite(item.price) && is) {
						return '￥' + item.price
					} else {
						return item.price
					}
			...
			viewDayAltStock(year, month, day) {
				let txt = ''
				let item = this.datePrice.find(item => {
					let today = item.date.replace(/\-/g, '/');
					let date = `${year}/${month}/${day}`;
			...
			btnFix(show) {
				...
				if (this.mode == 'date') {
					...
					let price = this.viewDayAlt(year, month, day, false);
					this.$emit('change', {
						year: year,
						month: month,
						day: day,
