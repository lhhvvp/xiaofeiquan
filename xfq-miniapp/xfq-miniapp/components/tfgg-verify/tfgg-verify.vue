<template>
	<view class="zhezhao" v-if="shows" @tap="shows=false">
		<view class="verifyBox" @touchend="onEnd">
			<view class="pintuBox">
				<image :src="img" class="pintuBg"></image>
			...
			<view class="huakuaiBox">
				...
				<movable-area :animation="true">
					<movable-view :x="x" direction="horizontal" @change="onMove">
						<view class="pintukuai" :style="{top:top+'px'}"><image :src="img" :style="{top:-lefttop+'px',left:-left+'px'}"></image></view>
					</movable-view>
				</movable-area>
	...
	export default {
		...
		data() {
			return {
				x: 0,//初始距离
				oldx:0,//移动的距离
				img:'https://oss.ylbigdata.com/admins/4885324cd67fe470fdc8e0c78a10effa.png',//显示哪张图片
				left:'',//随机拼图的最终X轴距离
				top:'',//拼图的top距离
				lefttop:'',//拼图内容的top距离
				shows:false
		...
		methods: {
			...
			shuaxinVerify(){
				...
				if(gl<=0.2){
					this.img='https://oss.ylbigdata.com/admins/4885324cd67fe470fdc8e0c78a10effa.png'
				}if(gl>0.2&&gl<=0.4){
					this.img='https://oss.ylbigdata.com/admins/f6702561e10dc41e137f77fa05172fe7.png'
				}if(gl>0.4&&gl<=0.6){
					this.img='https://oss.ylbigdata.com/admins/6c946408abc647cb7de16cc602a02f87.png'
				}if(gl>0.6&&gl<=0.8){
					this.img='https://oss.ylbigdata.com/admins/c5e5ca9ceecbf6e1ff593f9410197168.png'
				}if(gl>0.8&&gl<=1){
					this.img='https://oss.ylbigdata.com/admins/3f48dee7ec383213a380629ba0475a52.png'
				}
			},
			/* 滑动中 */
			onMove(e) {
			...
			onEnd() {
				...
				if(Math.abs(this.oldx-this.left)<=3){
