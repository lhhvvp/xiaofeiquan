<template>
	<view>
		<view class="nav-box">
			<my-nav :navList="nav" @current="current"></my-nav>
		</view>
			...
			<view class="item" v-for="(item,index) in list" :key="index" @click="navTo(item.id)">
				...
				<view class="bottom">
					...
					<view class="bottom-right">
						...
						<view class="number" style="margin: 4upx 0;">张数：{{item.number}}</view>
						<view class="count-price">时间：{{item.start}} - {{item.time_end_text}}</view>
					</view>
				</view>
				<view class="comment" v-if="item.order_status == 'iscomment'">
					<view class="comment-button" @click="Addcomment()">评论</view>
				</view>
			</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			navTo(id) {
				uni.navigateTo({
					url: "/pages/user/subscribe/detail?id=" + id
				})
			},
			current(index) {
				this.list = [];
				...
				this.$set(this.nav, 'current', index.index);
